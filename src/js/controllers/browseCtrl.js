...
app.controller('browseCtrl', function($scope, $sce, $http) {
  ...
  $scope.updateDataBase = (data) => {
    $http.put("api/allTheGames").then((res) => {
      console.log("DONE: ", res)
    });
  }

  $scope.getBrowseGamesList = () => {
    console.log("Getting games...")
    $http.get("./api/user-storage/gamesList").then((res) => {
      console.log(res.data.games)
      $scope.games = res.data.games
